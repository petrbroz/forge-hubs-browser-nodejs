...
async function getContents(hubId, projectId, folderId = null) {
    const contents = await _fetch(`/api/hubs/${hubId}/projects/${projectId}/contents` + (folderId ? `?folder_id=${folderId}` : ''));
...
async function getVersions(hubId, projectId, itemId) {
    const versions = await _fetch(`/api/hubs/${hubId}/projects/${projectId}/contents/${itemId}/versions`);
